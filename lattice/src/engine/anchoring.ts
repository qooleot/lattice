...
export function anchorsCandidate(
  ...
  // A verdict judged BEFORE this candidate existed can still anchor it — by AGREEMENT only:
  // the candidate must rule the witness the same way the human did. This is the regeneration
  // path's guarantee (regenerate rejects ledger-inconsistent candidates; golden trace A is the
  // canonical case: H3 is authored FROM the verdicts that pruned its predecessors and converges
  // with no further questions). A pre-registration verdict the candidate merely doesn't contradict
  // vacuously (aggregate absent) still never anchors — the aggregate filter above is unconditional.
