...
// Guarded transitions (t.requires set) whose guard predicate shape guardFieldOverride doesn't
// recognize — none of these can be chosen for the demo scenario. Named explicitly (no-silent-gaps
// discipline, design §5; same treatment as render/tests.ts's unrecognizedGuardShapeTransitions)
// rather than left to vanish quietly from consideration.
function unrecognizedGuardShapeTransitions(plan: GenPlan): string[] {
  const names: string[] = [];
  for (const a of plan.aggregates) {
    for (const t of a.transitions) {
      if (t.requires && guardFieldOverride(t.requires) === undefined) names.push(`${a.name}.${t.name}`);
    }
  }
  return names;
}

function skippedNote(plan: GenPlan): string {
  const skipped = unrecognizedGuardShapeTransitions(plan);
  return skipped.length
    ? `// SKIPPED (guard predicate shape not recognized — ineligible for the demo scenario): ${skipped.join(', ')}\n\n`
    : '';
}
