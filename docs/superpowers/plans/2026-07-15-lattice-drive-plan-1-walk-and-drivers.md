...
- Oracle semantics (design §3, verbatim): legal command rejected ⇒ violation `driver: impl rejected a spec-legal command`; illegal probe accepted ⇒ violation `driver: impl accepted a spec-illegal command`; superset ops — either outcome acceptable; guard evaluation = `evaluateCandidate` with the transition's `requires` wrapped as a `statePredicate` candidate over the scoped entities (`'permit'` = guard holds; `evalPred` is not exported — do not export it, wrap instead).
