...
- **Status:** Plan 1 (walk+drivers) landed 2026-07-16 — clean-impl validation green after two
  ...
  rediscovery campaign = plan 2. **Final: 11/13 drift classes rediscovered across three campaigns**
  (`docs/superpowers/specs/2026-07-15-lattice-drive-rediscovery-results.md`, Final Verdict) — the 2
  remaining misses (c06, c08) are MISSED-COVERAGE-BOUNDED, a measured budget ceiling (0/12 `pastDue`
  observations across a 4-rung ladder to 12,800 sequences), not a structural gap; criteria 2-5 all
  MET; no kill criterion triggered. Two clean-impl findings surfaced across the series and both
  landed as human-ruled spec amendments: the finalize/settle `requires` drop (`1fbf530`) and the
  `activePaidInFull` retirement (w6 finalize-on-active ruling, `eb375d9`).
...
| 4 | Pre-state oracle | **Scoped observe + tiered check frequency.** Per-step legality reads the REAL database, scoped to the target row + ref hops through the existing projection (no mirror — a maintained mirror is the §11.5 second truth, and desyncs worst exactly where it's tempting: real DBs with background jobs and concurrent writers). Full-tier sweeps at sequence end; per-step during shrinking; `--check-every` between. Cost model holds for remote/real-DB targets (O(1 query) per step). **Amendment 2026-07-16 (human ruling, c09 root-cause):** the full-tier sweep's sequence-boundary batching is structurally blind to a violation a later, itself-legal command resolves before the sweep runs — closed by a per-step Tier-1 check after every accepted/re-attributed/superset step. **Amendment 2026-07-16 (human ruling, scope widening):** a first attempt scoped that per-step check to the intention's nominally touched aggregate, which relocates rather than closes the gap for drivers that mutate a DIFFERENT aggregate than the one their intention names (`rollover`/`changeSeats`/`recordUsage`/`changePlanOp` are Subscription-bound yet mutate Invoice; `settle` is Invoice-bound yet bumps `Subscription.paid_invoice_count` and can recover `lifecycle_state`) — per-step scope is now ALL bound aggregates of the context (bounded-context-sized, not O(1 aggregate)), acceptable at this context's small scale; a DECLARED per-op mutation footprint (driver map states which aggregates an op can touch, scoping the per-step check to exactly that set) is recorded as the seam for large/remote targets where a full-context per-step read would be too costly. |
