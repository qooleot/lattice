...
| 6 | Cross-aggregate coupling | a `ref` with a state dependency | `A.state==Active ⇒ B.status∈{Paid} ∨ within grace` | active sub, long-unpaid invoice | relational + temporal | **deferred** — no template matches this shape, and `inState` is self-only: the candidate grammar rejects a non-`self` owner outright (`unsupported-owner`, `ast/grammar.ts`), so a *referenced* aggregate's state cannot be named in an invariant |
| 7 | Single-active (uniqueness) | an `@active` state on a child collection | `unique while active by (parent, key)` | two active per family | structural (Alloy) | **partial** — `templates.ts` #7 **seeds** `by (parent)` — the ref field alone, no key segment — at prior 0.4 (a hypothesis the loop tests), not adopted; the schema column's `(parent, key)` is aspirational |
| 8 | No period reuse / monotonic | a `@monotonic` field | `field only ↑` | double-billing a cycle | temporal | **implemented** — `templates.ts` #8, adopted. The tag is the only route by which the *engine* introduces a monotonic rule — `propose`/`regenerate` reject the kind as `not-elicitable` — though a hand-written `.lat` can author `monotonic <path>` directly |
| 9 | No orphan (referential integrity) | any same-context `ref` field (qualified cross-context refs excluded) | `every ref resolves` | invoice → deleted subscription | structural | **implemented** — `implied.ts`, one rule per owner covering all same-context `ref` fields, required and optional alike; an absent optional `ref` is exempt by evaluation (skipped, not convicted), not by exclusion from the rule |
