...
- **Nonlinear arithmetic** (proration = `price × days / period`) is undecidable; SMT may not terminate. **Restrict specs to linear arithmetic; push proration/rounding to property-based + differential testing against a reference impl.**
...
| 2 | **Billing = arithmetic worst case** | "routes to Apalache/cvc5" | proration is nonlinear (undecidable); real-time wants timed automata | High, domain-specific | linear-only; abstract time; property-test the arithmetic | Research + Eng |
